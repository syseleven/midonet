...
JVM_SEARCH_DIRS=$( bash -c "echo \
                   ...
                   /usr/$LIB_PATTERNS/jvm/adoptopenjdk-11-hotspot-jre \
                   /usr/$LIB_PATTERNS/jvm/adoptopenjdk-11-hotspot \
                   /usr/$LIB_PATTERNS/jvm/jre-11-openjdk")

check_for_java11() {
    [ "x" = "x$1" ] && return 1
    [ -x "$1" ] || return 1
    $1 -version 2>&1 | grep -q 'version "11.0'
}
...
for jdir in $JVM_SEARCH_DIRS; do
    check_for_java11 "$jdir/bin/java"
    if [ $? -eq 0 ]; then
        JAVA_HOME="$jdir"
        break
    fi
...
if [ -z "$JAVA_HOME" ] ; then
    echo "No suitable JVM found (at least v11.0 required)"
    exit 1
fi
