...
abstract class ConfigQuery(name: String) extends Subcommand(name) {
    ...
    val _renderOpts = ConfigRenderOptions.defaults().setFormatted(true)
    val _schemaRenderOpts = ConfigRenderOptions.defaults().setFormatted(true)
    ...
    val schema = opt[Boolean](
        ...
        "key documentation.")
    val jsonOutput = opt[Boolean](
        "json", short = 'j', default = Some(false), descr =
        "Output configuration options in JSON format.")
    ...
    def renderOpts: ConfigRenderOptions = if (schema.get.get) {
        _schemaRenderOpts.setOriginComments(true)
                         .setComments(true)
                         .setJson(false)
    } else {
        _renderOpts.setOriginComments(comments)
